...
const Settings = {
  //THIS IS IMPORTANT\\
  GUILD_ID: "993952487126085662", // THIS IS VERY IMPORTANT, THIS IS THE ID OF YOUR SERVER, SEARCH ON YOUTUBE HOW TO GET IT IF YOU DONT KNOW HOW
  OWNER_ID: "956325684102438953", // THIS IS VERY IMPORTANT, THIS IS THE ID OF YOUR PROFILE, SEARCH ON YOUTUBE HOW TO GET IT IF YOU DONT KNOW HOW
  Shoppy_API_KEY: "ztaf1lKCUzDukHbwmIMg2MbvR14Mu5Re4SpXVYakTIQBZ8XZHP", // Create a Shoppy account If you haven't, then go to https://shoppy.gg/user/settings and look for your "API-KEY".
  Whitelisted_Role_ID: "998189400150245377", // When users use the !whitelist or !rewhitelist command, they get this role.
  Blacklisted_Role_ID: "1000066045580677180", // When users who are not whitelist use the !rewhitelist command with a buyers Shoppy Order ID, they get this role.
  LogsChannel_Channel_ID: "993952489298727006", // Create a channel for logs when a user is whtelisted or uses a command!
  //----------------\\
  ...
  if(ShoppyIDS) {
    if (db.get(Settings.UsersDataStart+ThereKey)) {
      if (robloxUser === db.get(Settings.UsersDataStart+ThereKey)) {
        res.send("User Whitelist Found On Our Database") // CHANGE THIS TO MAKE THE WHITELIST MORE SECURE!
        return "Correct";
      }
      else
      {
      res.send("User Whitelist Not Found On Our Database")
      return "Nope.";
      }
    }
    else
   ...
   .then(function (res) { 
     ...
     else
     {
       if (res.data["confirmations"] === 1 && res.data["product"]["confirmations"]) {
         ...
         message.reply("_G.Key = '"+key+"'\nloadstring(game:HttpGet('https://botasaduahdaudha.herokuapp.com/script.lua', true))()") // Change PROJECTNAME to the project u made on heroku!!!
